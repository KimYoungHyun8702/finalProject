...
public class TimetableManagementServiceImpl implements TimetableManagementService {
	...
	public HashMap<String, Object> findMyTimeTableByJoin(String loginId) {
		...
		if(!stuRegister.equals("휴학") && !stuRegister.equals("군휴학")) {
			if(nowSemester == null) {
				// 만약 현재 로그인한 학생이 휴학, 군휴학 상태가 아닌 정상적인 재적상태라면에서 현재가 학기(수강기간) 기간이 아니면
				...
				map.put("message", "");	// 현재 로그인한 학생이 재적상태도 정상적인 재적상태이고 현재가 학기(수강기간)이면 msg "" 공백으로 map 담고
									// 바로 윗줄에서는 시간표 페이지에서 보여줄 내가 수강하고 있는 수강리스트를 담는다.
			}
		} else {
			map.put("stuRegisterTimeTable", stuRegister); // 현재 재적 상태가 휴학 상태이거나, 군휴학 이면 map에 담는다.
		}
		return map;
	}
